 ...
 * - Real-time event data loading and updating via React Query
 ...
 * 4. Form submission validates and calls appropriate mutation
...
export function EventModal({ isOpen, onClose, eventId, selectedDate }: EventModalProps) {
  ...
  useEffect(() => {
    if (event) {
      ...
      const endDate = event.endTime ? new Date(event.endTime) : new Date(event.startTime + 3600000);
