...
export const getMRRHistory = query({
  ...
  handler: async (ctx, args) => {
    ...
    for (let i = months - 1; i >= 0; i--) {
      ...
      // Calculate growth
      const previousMRR = mrrHistory.length > 0 ? mrrHistory[mrrHistory.length - 1].mrr : totalMRR;

      const growth = previousMRR > 0 ? ((totalMRR - previousMRR) / previousMRR) * 100 : 0;
...
export const getChurnAnalysis = query({
  handler: async (ctx) => {
    ...
    const cancelledMembers = members.filter((m) => m.subscriptionStatus === "cancelled");

    const churnedMembers = members.filter(
      (m) => m.subscriptionStatus === "expired" || m.status === "churned",
    );
    ...
    const monthlyChurnRate =
      activeMembers.length > 0 ? (recentChurns.length / activeMembers.length) * 100 : 0;

    // Analyze churn by tier
    const churnByTier = churnedMembers.reduce(
      (acc, member) => {
...
function calculateMemberMetrics(members: Doc<"members">[], cutoffTime: number) {
  const paidMembers = members.filter((m) => m.subscriptionStatus === "active");

  const newMembers = members.filter((m) => m.joinedDate >= cutoffTime);
  ...
  for (const tier of tiers) {
    ...
    for (const member of tierMembers) {
      const memberPayments = payments.filter(
        (p) => p.memberId === member._id && p.status === "succeeded",
      );
    ...
    tierMetrics[tier] = {
      ...
      mrr:
        monthlyCount * (tier === "founding_member" ? 2500 : tier === "early_bird" ? 4900 : 9900) +
