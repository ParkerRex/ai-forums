...
export const analyzeMembershipCounts = query({
  ...
  handler: async (ctx) => {
    ...
    const paidMembers = allMembers.filter((m) => m.tier && m.amountCents && m.amountCents > 0);

    // Count by subscription status for paid tiers
    const paidByStatus = paidMembers.reduce(
      (acc, m) => {
    ...
    const activePaidMembers = paidMembers.filter((m) => m.subscriptionStatus === "active");

    // Get members who should be active (active, cancelled but not expired, past_due)
    const shouldBeActivePaid = paidMembers.filter(
      (m) =>
