    ...
    for (const activeEmail of ACTIVE_MEMBER_EMAILS) {
      ...
      if (members.length > 1) {
        ...
        if (bestMember.status !== "active") {
          ...
          console.log(
            `✅ ${dryRun ? "[DRY RUN] " : ""}Updated best member to active: ${bestMember.firstName} ${bestMember.lastName} (${bestMember.email})`,
          );
          updatedCount++;
        }
        ...
        for (const member of members) {
          if (member._id !== bestMember._id && member.status !== "duplicate") {
            ...
            console.log(
              `🔄 ${dryRun ? "[DRY RUN] " : ""}Marked duplicate: ${member.firstName} ${member.lastName} (${member.email})`,
            );
          }
        }
      } else {
        ...
        if (member.status !== "active") {
          ...
          console.log(
            `✅ ${dryRun ? "[DRY RUN] " : ""}Updated to active: ${member.firstName} ${member.lastName} (${member.email})`,
          );
          updatedCount++;
        }
      }
