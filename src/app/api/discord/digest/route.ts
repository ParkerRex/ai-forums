...
export async function POST(request: NextRequest) {
	try {
		...
		const digestEntries = (await convex.action(
			api.discord.getDiscordDigestAction,
			{
		...
		if (channels && Array.isArray(channels) && channels.length > 0) {
			filteredEntries = digestEntries.filter(
				(entry: DiscordDigestEntry) =>
					channels.includes(entry.channelId) ||
					channels.includes(entry.channelName),
			);
		}
	...
	return NextResponse.json(
		{
			...
			usage:
				"POST /api/discord/digest with body: { guildId?, channels?, limit?, digestDate? }",
		},
		{ status: 405 },
	);
}
