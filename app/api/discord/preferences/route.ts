...
export async function GET() {
  try {
    ...
    const prefs = memberData[0].newsPreferences as {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const currentPrefs =
      (memberData[0]?.newsPreferences as { discord?: { enabled: boolean } } | null) || {};
    ...
    await db.update(members).set({ newsPreferences: newPrefs }).where(eq(members.id, member.id));
