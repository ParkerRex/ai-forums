...
import { db } from "@/db";
import { commentReports, comments, members, posts, categories } from "@/db/schema";
...
export async function GET(request: NextRequest) {
	...
	try {
		...
		const enrichedReports = reports.map((report) => {
			const commentAuthor = report.commentMemberId
				? membersById.get(report.commentMemberId)
				: null;
			const reporter = report.reporterId
				? membersById.get(report.reporterId)
				: null;
			const category = report.postCategoryId
				? categoriesById.get(report.postCategoryId)
				: null;
