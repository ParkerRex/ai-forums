...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (existingEvent.status === "cancelled" || existingEvent.status === "completed") {
      return NextResponse.json({ error: "Event is no longer accepting RSVPs" }, { status: 400 });
    }
    ...
    if (action === "rsvp") {
      ...
      if (existingEvent.maxAttendees && newAttendees.length >= existingEvent.maxAttendees) {
        // Add to waitlist
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    return NextResponse.json({ error: "Failed to get RSVP status" }, { status: 500 });
  }
}
