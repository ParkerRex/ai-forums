...
async function analyzeDataIntegrity() {
  ...
  try {
    ...
    const postsWithoutAuthors = await client
    ...
    if (!postsWithoutAuthors) {
      ...
      if (postsWithIssues.length > 0) {
        ...
        postsWithIssues.slice(0, 5).forEach((post) => {
          console.log(
            `- ID: ${post._id}, Title: "${post.title}", Created: ${new Date(post.createdAt).toISOString()}`,
          );
        });
      }
    }
    ...
    const membersWithFakeEmails = allMembers.filter(
      (member) =>
        member.email &&
    ...
    if (membersWithFakeEmails.length > 0) {
      ...
      membersWithFakeEmails.slice(0, 5).forEach((member) => {
        console.log(
          `- ID: ${member._id}, Name: "${member.firstName} ${member.lastName}", Email: ${member.email}`,
        );
      });
    }
    ...
    if (orphanedComments.length > 0) {
      ...
      orphanedComments.slice(0, 5).forEach((comment) => {
        console.log(
          `- ID: ${comment._id}, PostId: ${comment.postId || "null"}, Content: "${comment.content.substring(0, 50)}..."`,
        );
      });
    }
