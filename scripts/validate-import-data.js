...
// Read all the files
const posts = JSON.parse(fs.readFileSync(path.join(__dirname, '../migration-data/posts-import-cleaned.json'), 'utf8'));
const comments = JSON.parse(fs.readFileSync(path.join(__dirname, '../migration-data/comments-import-cleaned.json'), 'utf8'));
const userMapping = JSON.parse(fs.readFileSync(path.join(__dirname, '../migration-data/user-mapping.json'), 'utf8'));
...
const brokenCommentChains = comments.filter(c => 
  c.parentCommentSkoolId && !commentIds.has(c.parentCommentSkoolId)
);
...
if (brokenCommentChains.length > 0) issues.push(`${brokenCommentChains.length} broken comment chains`);
