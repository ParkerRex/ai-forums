...
async function main() {
  ...
  const verifiedMapping = JSON.parse(
    fs.readFileSync(path.join(__dirname, 'migration-data/verified-import-results/verified-post-id-mapping.json'), 'utf8')
  );
  const postIdMap = verifiedMapping.postIdMap;
  
  // Load comments
  const comments = JSON.parse(
    fs.readFileSync(path.join(__dirname, 'migration-data/import-ready-2025-07-26/comments-to-import.json'), 'utf8')
  );
  ...
  for (let i = 0; i < comments.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      validComments.forEach(c => errors.push(`Comment batch error: ${error.message}`));
    }
  }
