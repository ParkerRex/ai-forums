...
console.log(`  - ${postBatches.length} post batches (${postBatches.reduce((sum, batch) => sum + batch.length, 0)} total posts)`);
console.log(`  - ${commentBatches.length} comment batches (${commentBatches.reduce((sum, batch) => sum + batch.length, 0)} total comments)`);
...
console.log('  - Post batches: migration-data/post-batch-0.json through post-batch-' + (postBatches.length - 1) + '.json');
console.log('  - Comment batches: migration-data/comment-batch-0.json through comment-batch-' + (commentBatches.length - 1) + '.json');
...
console.log('After importing posts, you\'ll need to create a mapping of Skool post IDs to Convex post IDs.');