...
async function runMigration() {
  ...
  try {
    ...
    const result = await client.mutation(
      api.migrations.migrate_post_attachments.migratePostAttachments,
      {
        batchSize: 100,
      },
    );
    ...
    const linkResult = await client.mutation(
      api.migrations.migrate_post_attachments.migrateLinkedPostAttachments,
      {
        batchSize: 100,
      },
    );
    ...
    console.log(
      `Link migration completed. Processed ${linkResult.processedCount} posts with link images.`,
    );
