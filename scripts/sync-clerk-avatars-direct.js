    ...
    for (let i = 0; i < members.length; i += batchSize) {
      ...
      await Promise.all(batch.map(async (member) => {
        try {
          ...
          console.error(`❌ Error syncing avatar for ${member.firstName} ${member.lastName}:`, error.message);
          errors++;
        }
      }));
      
      // Progress update
      console.log(`📈 Progress: ${Math.min(i + batchSize, members.length)}/${members.length} members processed`);
