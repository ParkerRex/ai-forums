...
const dataPath = path.join(__dirname, '../migration-data/_skool-complete-troublefreeai-2025-06-27.json');
  ...
  data.posts.forEach(post => {
    if (post.postType === 'generic' && post.rootId === post.id) {
      ...
      const transformedPost = {
        ...
        createdAt: convertSkoolTimestamp(post.createdAt),
        updatedAt: convertSkoolTimestamp(post.updatedAt),
        authorEmail: post.user?.email || userIdMap.get(post.userId)?.email || `unknown@imported.com`,
...
if (data.comments) {
  data.comments.forEach(comment => {
    const transformedComment = {
      ...
      createdAt: convertSkoolTimestamp(comment.created_at || comment.createdAt),
      updatedAt: convertSkoolTimestamp(comment.updated_at || comment.updatedAt),
      authorEmail: comment.user?.email || userIdMap.get(comment.user_id || comment.userId)?.email || `unknown@imported.com`,
      postSkoolId: comment.root_id || comment.rootId,
      parentCommentSkoolId: (comment.parent_id || comment.parentId) !== (comment.root_id || comment.rootId) ? (comment.parent_id || comment.parentId) : null,
      status: 'active',
...
comments.forEach(comment => {
  ...
  while (currentComment.parentCommentSkoolId && depth < 10) { // Max depth safety
    depth++;
