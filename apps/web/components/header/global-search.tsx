...
function highlightMatch(text: string, searchTerm: string): React.ReactNode {
  ...
  return parts.map((part, index) =>
    regex.test(part) ? (
      <mark
        key={index}
        className="bg-muted/50 dark:bg-muted/30 text-foreground rounded px-0.5 font-medium"
      >
        {part}
      </mark>
    ) : (
...
function SearchResultItem({
  ...
  const item = (
    <CommandItem
      ...
      className={`data-[selected=true]:bg-muted data-[selected=true]:text-foreground flex items-center gap-3 p-3 ${result.restricted ? "opacity-60" : ""}`}
    >
