    ...
    try {
      ...
      while (retries > 0) {
        try {
          ...
          console.warn(
            `Failed to generate upload URL, retrying... (${retries} attempts left)`,
          );
      ...
      if (!uploadData) {
        throw new Error(
          "Failed to generate upload URL after multiple attempts",
        );
      }
      ...
      while (retries > 0 && !uploadSuccess) {
        try {
          ...
          if (retries === 0) throw error;
          console.warn(
            `Upload attempt failed, retrying... (${retries} attempts left)`,
          );
