...
export default internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    console.log(`Starting migration batch: limit=${limit}, startAfter=${args.startAfter || 'beginning'}`);
    ...
    const legacyMembers = allMembers
      .filter((member) => !member.externalId)
      .slice(0, limit);
    ...
    for (let i = 0; i < legacyMembers.length; i++) {
      ...
      try {
        console.log(`Processing member ${i + 1}/${legacyMembers.length}: ${member.email} (${member._id})`);
