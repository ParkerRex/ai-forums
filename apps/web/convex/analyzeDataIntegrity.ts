...
export const analyzeData = query({
  ...
  handler: async (ctx) => {
    ...
    const postsWithoutAuthors = allPosts.filter(post => 
      !post.memberId || !memberIds.has(post.memberId)
    );
    
    // 2. Members with placeholder emails
    const membersWithPlaceholderEmails = allMembers.filter(member => 
      member.email && (
    ...
    const commentsWithoutAuthors = allComments.filter(comment => 
      !comment.memberId || !memberIds.has(comment.memberId)
    );
