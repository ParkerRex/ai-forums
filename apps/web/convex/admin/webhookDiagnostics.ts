...
export const getWebhookDiagnostics = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Group by event type
    const eventsByType = webhookEvents.reduce((acc, event) => {
    ...
    const membersWithSubscriptions = recentMembers.filter(m => 
      m.stripeCustomerId && m.subscriptionStatus === "active"
    );
    ...
    return {
      ...
      diagnosis: {
        ...
        recentWebhookActivity: webhookEvents.some(e => 
          e._creationTime > Date.now() - 24 * 60 * 60 * 1000 // Last 24 hours
        ),
        subscriptionFlowWorking: membersWithSubscriptions.length > 0,
        possibleIssues: [
          ...(totalEvents === 0 ? ["No webhook events received - check Stripe webhook configuration"] : []),
          ...(failedEvents > 0 ? [`${failedEvents} failed webhook events - check error logs`] : []),
          ...(totalEvents > 0 && processedEvents === 0 ? ["Webhooks received but none processed - check webhook processing logic"] : []),
          ...(membersWithSubscriptions.length === 0 ? ["No members with active subscriptions - check payment flow"] : []),
        ],
      },
    };
  },
...
export const checkMissingWebhookEvents = query({
  ...
  handler: async (ctx) => {
    ...
    const membersWithStripeData = recentMembers.filter(m => 
      m.stripeCustomerId && m.stripeCustomerId !== ""
    );
    
    // Get all webhook events
    const allWebhookEvents = await ctx.db
      .query("stripeWebhookEvents")
      .collect();
    
    // Check for potential missing events
    const potentialMissingEvents = membersWithStripeData.filter(member => {
      // Check if there's a corresponding webhook event for this member's customer
      const hasWebhookEvent = allWebhookEvents.some(event => 
    ...
    return {
      ...
      recommendation: potentialMissingEvents.length > 0 
