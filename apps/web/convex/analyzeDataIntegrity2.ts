...
export const findInvalidMemberReferences = query({
  ...
  handler: async (ctx) => {
    ...
    const postsWithInvalidMembers = allPosts.filter(post => 
      post.memberId && !validMemberIds.has(post.memberId)
    );
    
    // Check comments for invalid member IDs
    const allComments = await ctx.db.query("comments").collect();
    const commentsWithInvalidMembers = allComments.filter(comment => 
      comment.memberId && !validMemberIds.has(comment.memberId)
    );
