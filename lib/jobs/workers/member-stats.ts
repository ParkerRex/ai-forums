import { db } from "@/db";
import { members, posts, comments, votes } from "@/db/schema";
...
export async function recalculateMemberStats() {
	...
	for (const member of allMembers) {
		...
		const commentCountResult = await db
			...
			.from(comments)
			.where(
				and(eq(comments.memberId, member.id), eq(comments.status, "active")),
			);
		...
		const netVoteCount =
			Number(postVotesResult[0]?.total || 0) +
			Number(commentVotesResult[0]?.total || 0);
