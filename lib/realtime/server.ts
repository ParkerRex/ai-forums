...
class RealtimeServer {
  ...
  private handleConnection(ws: WebSocket, req: IncomingMessage) {
    ...
    console.log(`Client connected: ${memberId || "anonymous"} (${this.clients.size} total)`);
  ...
  private pingClients() {
    ...
    for (const [ws, client] of this.clients) {
      if (now - client.lastPing > timeout) {
        ...
        console.log(`Client timeout: ${client.memberId || "anonymous"}`);
        ws.terminate();
        this.handleDisconnect(ws);
  ...
  getStats() {
    return {
      ...
      channels: Object.fromEntries(
        Array.from(this.channelSubscribers.entries()).map(([channel, subscribers]) => [
          channel,
          subscribers.size,
        ]),
      ),
...
export function publishToChannels(channels: string[], event: Omit<RealtimeEvent, "channel">) {
