...
async function uploadViaServer(file: File, options: UploadOptions = {}): Promise<UploadResult> {
  ...
  try {
    ...
    // Upload via our API endpoint using FormData
    ...
    if (file.type.startsWith("video/") && !options.skipThumbnail) {
      ...
      if (thumbnailDataUrl) {
        ...
        try {
          const thumbnailResult = await uploadViaServer(thumbnailFile, {
            ...options,
            skipThumbnail: true,
          });
 ...
 */
async function uploadViaDirect(file: File, options: UploadOptions = {}): Promise<UploadResult> {
  const { onProgress } = options;
          ...
          if (file.type.startsWith("video/") && !options.skipThumbnail) {
            try {
              ...
              if (thumbnailDataUrl) {
                ...
                try {
                  const thumbnailResult = await uploadViaDirect(thumbnailFile, {
                    ...options,
                    skipThumbnail: true,
                  });
                  thumbnailUrl = thumbnailResult.url;
              ...
              console.error("Failed to extract video thumbnail:", thumbnailErr);
            }
          }
...
export async function uploadMedia(file: File, options: UploadOptions = {}): Promise<UploadResult> {
  ...
  try {
    return await uploadViaDirect(file, options);
  } catch (error) {
...
export function validateDocumentFile(file: File): { valid: boolean; error?: string } {
  if (!ALLOWED_DOCUMENT_TYPES.includes(file.type)) {
    return {
      valid: false,
      error:
        "File type not supported. Please upload PDF, Word, PowerPoint, Excel, text, or markdown documents.",
    };
  }
...
export async function extractMediaDimensions(
  file: File,
): Promise<{ width: number; height: number; aspectRatio: number }> {
  if (file.type.startsWith("image/")) {
    ...
    return { width: 850, height: 1100, aspectRatio: 850 / 1100 };
  } else if (
