...
export function useRealtime(options: UseRealtimeOptions = {}) {
  ...
  const subscribe = useCallback((newChannels: string | string[]) => {
    const channelList = Array.isArray(newChannels) ? newChannels : [newChannels];
  ...
  const unsubscribe = useCallback((channelsToRemove: string | string[]) => {
    const channelList = Array.isArray(channelsToRemove) ? channelsToRemove : [channelsToRemove];
  ...
  useEffect(() => {
    ...
    const toUnsubscribe = Array.from(currentChannels).filter((c) => !newChannels.has(c));
