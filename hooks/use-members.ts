...
async function fetchOnlineMembers(): Promise<Member[]> {
  ...
  return data.items.filter(
    (member: Member) => member.lastOnline && new Date(member.lastOnline).getTime() > fiveMinutesAgo,
  );
}

async function updateMember(memberId: string, data: UpdateMemberData): Promise<Member> {
  const response = await fetch(`/api/members/${memberId}`, {
    method: "PATCH",
...
async function fetchMemberPosts(memberId: string, cursor?: string): Promise<MemberPostsResponse> {
