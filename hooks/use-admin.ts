...
import type { MemberDetailsResponse, MembershipStats, MemberWithStatus } from "@/types/admin";
...
type WebhookHealth = {
  ...
  metrics: {
    ...
    eventTypeMetrics: Record<string, { total: number; processed: number; failed: number }>;
  };
};
...
async function fetchAdminMemberDetails(memberId: string): Promise<MemberDetailsResponse> {
...
async function fetchReportedComments(status?: string, limit?: number): Promise<EnrichedReport[]> {
  const params = new URLSearchParams();
 ...
 */
export function useAdminMembers(options?: { status?: string; search?: string; limit?: number }) {
  return useQuery({
